...
function handlerNoDirectroyError (err, ftp, file, files, callback) {
  var hasNoDirectoryError = err.message.indexOf('No such file or directory')

  if (!hasNoDirectoryError) callback(err)
  console.log('try create directory!')
  ftp.client.mkdir(path.parse(file.to).dir, true, function (err) {
    if (err) throw err
...
function upload(client, files, callback) {
  ...
  client.upload(file, function(err) {
    if (err) {
      util.failure(file)
      handlerNoDirectroyError(err, client, file, files, callback)
    } else {
      util.success(file)
      if (files.length > 0) {
...
function customDest(file, rule) {
  ...
  if (matched) {
    if (typeof rule.dest === 'function')
      file.dest = rule.dest.call(null, matched);
    else
...
function Deploy(options) {
  ...
  this.serverInfo = takeServerInfo(options.server)
  ...
  this.initClient(this.serverInfo)
}
...
Deploy.prototype.exec = function(files) {
  ...
  function callback(err) {
    ...
    self.end()
    // self.initClient(self.serverInfo)
  }
